...
import eeg_lstm_model, visual_resnet
from torch.utils.data import DataLoader, TensorDataset

device = torch.device("cuda" if torch.cuda.is_available() else "cpu") 

class LEMCIMModel(nn.Module):

    def __init__(self, eeg_input_shape, visual_input_shape, n_classes=39):
        super(LEMCIMModel, self).__init__()

        # EEG model
        self.eeg_model = eeg_lstm_model.LSTMModel()
        self.eeg_model.fc2 = nn.Identity()
